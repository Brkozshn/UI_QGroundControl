...
def arm_and_takeoff(aTargetAltitude):
    ...
    while True:
        ...
        if drone.location.global_relative_frame.alt >= aTargetAltitude * 0.95:  # Trigger just below target alt.
...
"""
Functions to move the drone to a specified position (as opposed to controlling movement by setting velocity components).
...
* goto - A convenience function that can use drone.simple_goto (default) or 
...
"""
Functions that move the drone by specifying the velocity components in each direction.
The two functions use different MAVLink commands. The main difference is
that depending on the frame used, the NED velocity can be relative to the drone
orientation.
...
"""
Fly a triangular path using the standard drone.simple_goto() method.
...
#goto_position_target_local_ned(5,5,-1)    ## x,y,z metre cinsinden ilerler. Bu başlangıç noktasını referans alıyor.

send_ned_velocity(5, 0, -1, 5)   # 5 saniye boyunca x yönde 5 m/s y de 0 z de -1

condition_yaw(180, True)      #True ise kendine göre 180 derece dön

condition_yaw(0, False)    #kuzeye dönecek drone

#goto_position_target_relative_ned(10, -5, -5)  #kendini refereans aldı. en kullanışlı şekli bu